...
void create_graph()
{
        ...
        for(i=1; i<=max_edges; i++)
        {
                ...
                scanf("%d",&wt);
                if( origin >= n || destin >= n || origin<0 || destin<0) //0,1,..n-1 : n
                {
                        printf("\nInvalid edge!\n");
                        i--;
                }
...
void insert_pque(int i,int j,int wt)
{
        ...
        if( front == NULL || tmp->weight < front->weight )//beginning
        {
                tmp->link = front;
                front = tmp;
        }
...
void make_tree(struct edge tree[])
{
        ...
        while( !isEmpty_pque( ) && count < n-1 )
        {
                ...
                v1 = tmp->u;
                v2 = tmp->v;
                //finding for absolute parents
                while( v1 !=NIL )
                { //last iteration v1 nill . root_v1 lagging contains father of v1
                        root_v1 = v1;
                        v1 = father[v1];
                }
                while( v2 != NIL  )
